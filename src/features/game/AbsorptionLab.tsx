...
import { ABSORPTION_STEP_MS, observationLabel, recordAbsorption, type AbsorptionPhase } from "./absorption";
...
function StripComparisonStage({ materials, phase }: { materials: MaterialDefinition[]; phase: AbsorptionPhase }) {
...
export function AbsorptionLab({ save, onSave, onDone, preview = false }: {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setPhase(next[phase as Exclude<AbsorptionPhase, "idle" | "done">]), ABSORPTION_STEP_MS);
  ...
  const start = () => guard(() => { setObservations({}); setAccepted(false); setPhase("prepare"); });
  ...
  return <div className={`screen ${base.screen} ${styles.screen}`} data-focus={focusStep}>
    <header className={base.header}>
      ...
      <div className={base.heading}><h1>ห้องที่ 3 : การดูดซับน้ำของวัสดุ</h1><p>วัสดุชนิดใดดูดซับน้ำได้มากที่สุด?</p></div>
    </header>
    ...
    <section className={`${base.panel} ${base.experiment} ${styles.experiment}`} aria-label="ขั้นตอนการทดสอบการดูดซับน้ำ">
      ...
      <AbsorptionScale3D materials={LAB_MATERIALS} phase={phase}>
        <div className={styles.demonstration}><StripComparisonStage materials={LAB_MATERIALS} phase={phase} /><div className={styles.stageCopy}>
    ...
    <aside className={`${base.panel} ${base.materials}`}>
      ...
      <div className={`${base.materialList} ${styles.materialLegend}`}>{LAB_MATERIALS.map((item) => <article key={item.id} className={styles.materialLegendCard}>
        <img src={asset(`materials/${item.image}`)} alt="" /><span>{shortMaterialName(item)}</span>
      </article>)}</div>
    </aside>
    ...
    <section className={`${base.panel} ${base.recordPanel}`}>
      <div className={base.recordButtons}>
        <button data-saved={accepted} disabled={accepted ? false : !done || !allObserved} onClick={accepted ? onDone : record}><LabIcon name={accepted ? "play" : "save"} />{accepted ? "กลับไปเลือกห้องทดลอง" : !done ? "เริ่มทดลองก่อน" : !allObserved ? `เลือกผลให้ครบ (${observedCount}/5)` : "บันทึกคำตอบ"}</button>
    ...
    <dialog ref={recordsDialog} className={base.dialog} onCancel={() => setShowRecords(false)} onClose={() => setShowRecords(false)} aria-labelledby="water-records-title">
      ...
      <p className={base.recordNote}>ผลนี้เป็นหลักฐานเชิงประจักษ์จากแบบจำลองเพื่อฝึกเปรียบเทียบวัสดุ ไม่ใช่ค่าทดสอบวัสดุจริง{preview ? " · บันทึกเฉพาะรอบทดลองอิสระนี้" : ""}</p>
